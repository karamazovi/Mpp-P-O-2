...
Parámetros del artículo:
  ...
  Sim    : dt=2e-5s, T_total=2.4s, G=1000W/m², T=25°C
"""
...
with open(csv_path, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['tiempo_s', 'Vpv_V', 'Ipv_A', 'Ppv_W', 'D', 'Vco_V', 'IL_A'])
    for row in zip(t_arr, Vpv_arr, Ipv_arr, Ppv_arr, D_arr, Vco_arr, IL_arr):
...
V_curva, _, P_curva = panel.curva_pv(irradiancia, temperatura)
